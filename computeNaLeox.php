	...
	if ( $numberofrooms > 0 ){ // Check if there is Number of rooms to be reserve
		...
		if ( $totalNights >= 6 ){ // GDR
			...
			if ( $cntBO > 0 ){
				...
				//$boDateRange = $getDateRangeText( $startBO, $endBO, 1 );
...
//Check Breakfast Package				
		...
		if ( $RTID == 1 && $PSBF == "Y" ){ $foundBfast = 1; $isPSBF = 1; }
		if ( $RTID == 15 && $FABF == "Y" ){ $foundBfast = 1; $isFABF = 1; }
		if ( $RTID == 5 && $LVBF == "Y" ){ $foundBfast = 1; $isLVBF = 1; }
		if ( $RTID == 6 && $LSBF == "Y" ){ $foundBfast = 1; $isLSBF = 1; }
		if ( $RTID == 7 && $WEBF == "Y" ){ $foundBfast = 1; $isWEBF = 1; }
		if ( $RTID == 8 && $OBKBF == "Y" ){ $foundBfast = 1; $isOBKBF = 1; }
		if ( $RTID == 9 && $TKKBF == "Y" ){ $foundBfast = 1; $isTKKBF = 1; }
		if ( $RTID == 10 && $RBBF == "Y" ){ $foundBfast = 1; $isRBBF = 1; }
		if ( $RTID == 11 && $PHBF == "Y" ){ $foundBfast = 1; $isPHBF = 1; }
		if ( $RTID == 12 && $QVBF == "Y" ){ $foundBfast = 1; $isQVBF = 1; }
		if ( $RTID == 13 && $SPABF == "Y" ){  $foundBfast = 1; $isSPABF = 1; }		
	}	
}
...
if ( $foundBfast == 1 ){ // Breakfast code 	
	...
	if ( $isPSBF == 1 ){
		...
		$bName = "Poolside Room";
		$bPrice = 30;
		$bfastPrice = ( $bPrice * $numberofrooms * $roomNights );
		$bfastTotal = $bfastTotal + $bfastPrice;
		echo "<tr><td colspan=2 style='text-align:right;'>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;", "$bName $ $bPrice x $roomNights ",
			 AddRemoveSs( "night", $roomNights ), " x  $numberofrooms ", AddRemoveSs( "room", $numberofrooms),
